...
export default function UnivEtcPage() {
  ...
  // 서경대는 단독 상세 페이지(/univ/seokyeong/)로 분리했지만, 이 페이지의 '7개 대학 묶음' 비교표에는 계속 포함한다.
  const univs = UNIVERSITIES.filter((u) => u.path === PATH || u.id === 'skuniv');
    ...
    <PageShell path={PATH} width="wide">
      ...
      <article className="prose">
        ...
        <section aria-labelledby="sec-5">
          <h2 id="sec-5">서경대학교 미용예술대학 (헤어·메이크업·코스메틱)</h2>
          <p>
            서울 성북구에 있는 4년제로, 미용예술대학 안에 헤어디자인학과·메이크업디자인학과·코스메틱뷰티매니지먼트학과 3개 학과를 운영합니다. 수시 실기우수자 전형에 수능 최저학력기준이 없고, 학과와 유형에 따라 실기 반영 비율이 40%에서 80%까지 갈립니다. 특히 헤어디자인학과는 같은 학과를 A(실기 80%)와 B(교과 60%) 두 유형으로 나눠 모집하는 것이 특징입니다.
            메이크업디자인학과는 <Link href="/practice/artmask/">아트마스크</Link>·뷰티일러스트 계열 작품 실기, 코스메틱뷰티매니지먼트학과는 작품 대신 화장품 품목을 분석해 답하는 실기구술 방식입니다.
          </p>
          <p>
            모집인원·경쟁률·합격자 등급컷과 실기고사 일정, 준오뷰티 등 계약학과 채용조건형 전형까지 정리한 내용은 <Link href="/univ/seokyeong/">서경대 미용예술대학 입시 총정리</Link> 페이지에서 확인하세요.
          </p>
        </section>
      ...
      <div style={{ margin: '56px 0' }}>
        <FaqSection
          ...
          items={[
            ...
            { q: '서경대 메이크업디자인학과 실기는 어떻게 준비하나요?', a: '아트마스크·뷰티일러스트 계열의 작품 실기가 중심입니다. 수시 실기우수자 전형은 1단계 실기 100%로 모집인원의 300%를 선발한 뒤 2단계에서 교과 20% + 실기 80%를 반영합니다. 학과별 실기 과목과 일정은 서경대 미용예술대학 상세 페이지와 입학처 모집요강에서 확인하세요.' },
      ...
      <RelatedLinks
        paths={['/diagnosis/', '/univ/', '/univ/seokyeong/', '/univ/sungshin/', '/univ/seowon/', '/major/cosmetic/']}
